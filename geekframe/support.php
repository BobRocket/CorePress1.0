...
function corepress_webp_file_display($result, $path)
{
...
function CorePress_replace_avatar($avatarUrl, $id_or_email, $arrs)
{
    global $set, $wp_filesystem, $upload;
    ...
    if ($set['user']['upload_avatar'] == 1) {
        ...
        if (filter_var($id_or_email, FILTER_VALIDATE_EMAIL)) {
            $currentUser = get_user_by('email', $id_or_email);
            if ($currentUser) {
                $filename = $currentUser->ID . '.jpg';
            }
        } else {
            $filename = $id_or_email . '.jpg';
        }
        if (isset($filename)) {
            if ($wp_filesystem->exists(AVATAR_DIR . $filename)) {
                return $upload['baseurl'] . '/corepress_avatar/' . $filename . '?v=' . filemtime(AVATAR_DIR . $filename);
            }
        }
    }
    if ($set['optimization']['gravatarsite'] == 'v2ex') {
        $cdnurl = 'cdn.v2ex.com/gravatar';
        $avatarUrl = corepress_replace_gravatar($cdnurl, $avatarUrl);
    ...
    } elseif ($set['optimization']['gravatarsite'] == 'qiniu') {
        $cdnurl = 'dn-qiniu-avatar.qbox.me/avatar';
        $avatarUrl = corepress_replace_gravatar($cdnurl, $avatarUrl);
    }elseif ($set['optimization']['gravatarsite'] == 'loli') {
...
add_filter('get_avatar_url', 'CorePress_replace_avatar', 20, 3);
...
function copay_footer_admin($text)
{
    ...
    if ($set['info']['themeupdate'] == 1) {
        ...
        if ($set['info']['newversion'] != THEME_VERSION) {
            $url = '，<a href="' . $set['info']['downurl'] . '" target="_blank">立即更新</a> 不会更新请查看<a href="https://www.yuque.com/applek/corepress/setup" target="_blank">更新帮助</a>';
        }
    } else {
        $url = '，已关闭更新';
    }
...
function corepress_comment_face($incoming_comment)
{
    ...
    foreach ($match[1] as $facename) {
        if (file_exists($path . $facename . '.gif')) {
            $incoming_comment = str_replace("[f={$facename}]", '<img src="' . THEME_IMG_PATH . '/face/' . $facename . '.gif" width="24">', $incoming_comment);
        }
    }
    return $incoming_comment;
}
    ...
    if (get_user_option('rich_editing') == 'true') {
        ...
        <script>
            $(document).ready(function () {
                layer.config({
                    extend: 'corepress/style.css?v=4.8', //加载您的扩展样式,它自动从theme目录下加载这个文件
...
function corePress_lazyload($content)
{
    ...
    foreach ($images[0] as $item) {
        ...
        $replace_str = str_replace($need_replace_str, 'data-original="' . $class[1] . '"' . 'src="' . file_get_img_url('loading.gif') . '"', $replace_str);
...
if ($set['optimization']['revisions_to_keep'] == 1) {
...
function corepress_searchfilter($query)
{
    if ($query->is_search) {
...
add_filter('pre_get_posts', 'corepress_searchfilter');
